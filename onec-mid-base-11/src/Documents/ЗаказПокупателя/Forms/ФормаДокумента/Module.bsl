...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//добавление "Согласованная скидка"
	Если Элементы.Найти("СогласованнаяСкидка") = Неопределено Тогда
		
		ДобавляемыеРеквизиты = Новый Массив;
		ТипРеквизита = Новый ОписаниеТипов ("Число");
		РеквизитКонтактноеЛицо = Новый РеквизитФормы("СогласованнаяСкидка", ТипРеквизита, "", "Согласованная скидка");
		...
		НовыйЭлемент.УстановитьДействие("ПриИзменении", "ПриИзмененииСогласованнаяСкидка");
		
	КонецЕсли;
	
	Команда = Команды.Добавить("ПересчетСуммы");
	Команда.Заголовок = "Пересчет суммы";
	Команда.Действие = "КомандаПересчетСуммы";
	
	КнопкаФормы = Элементы.Добавить("КомандаПересчетСуммы", Тип("КнопкаФормы"), Элементы.ГруппаШапкаЛево);
	КнопкаФормы.ИмяКоманды = "ПересчетСуммы";
...
Асинх Процедура ПриИзмененииСогласованнаяСкидка(Элемент)
	...
	//Задать вопрос при изменении
	Если Объект.Товары.Количество() = 0 Тогда
		Возврат;
	ИначеЕсли Объект.Услуги.Количество() > 0 Тогда
		Возврат;
	Иначе
		ТекстВопроса = "Изменен процент согласованной скидки. Пересчитать таблицу?";
...
&НаКлиенте
Процедура КомандаПересчетСуммы (Команда)
	
	//команда "пересчет суммы"
	ПересчетСуммыСтроки();
    РассчитатьСуммуДокумента();
